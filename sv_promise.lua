...
local do_async = function(callback)
    if kPromise.async then
        kPromise.async(callback)
    else
...
transition = function(promise, state, value)
    if promise.state == state or promise.state ~= State.PENDING or (state ~= State.FULFILLED and state ~= State.REJECTED) or value == nil then return end
    promise.state = state
    promise.value = value
    run(promise)
end

function kPromise:next(on_fulfilled, on_rejected)
...
function kPromise.new(callback)
    local instance = {
        queue = {}
    }

    setmetatable(instance, kPromise.mt)
...
function kPromise:catch(callback)
    return self:next(nil, callback)
end

function kPromise:resolve(value)
    fulfill(self, value)
end

function kPromise:reject(reason)
    reject(self, reason)
end

function kPromise.update()
    while true do
        local async = table.remove(queue, 1)
        if not async then break end
        async()
    end
end

-- resolve when all promises complete
function kPromise.all(...)
    local promises = {...}
...
function kPromise.race(...)
    local promises = {...}

    local promise = kPromise.new()

    kPromise.all(...):next(nil, function(value)
...
hook.Add('Think', 'kPromise#Think', function()
    kPromise.update()
end)
